...
Getting it that quiet took some work.

## Why K3s ended up here

I did not choose Kubernetes because these workloads demand it. A handful of containers would run fine under Docker Compose. I chose K3s because I wanted a real Kubernetes API without turning the lab into a full-time job.
...
The cluster is small on purpose. The nodes are ordinary machines already suited to staying on. I would rather understand a modest setup than operate a miniature copy of a cloud platform. There is no service mesh and no internal developer platform. Most workloads are a Deployment or StatefulSet, a Service, and an Ingress.

That is enough complexity for a house.

## Git is how I remember what I did
...
This is not about fashionable GitOps vocabulary. I have an unreliable memory. If I change a port from the command line at midnight, I will not remember it later. If I change a manifest, the diff tells me what happened and gives me a way back.

I still use `kubectl` for inspection. I try not to use it as a configuration database.

My usual deployment loop is boring on purpose:

1. Edit the manifest locally.
2. Review the diff.
3. Apply the change.
4. Watch the rollout and events.
5. Confirm the service from a browser or DNS client, not from inside the pod.
...
## Persistent storage is the limit
...
I keep persistent storage simple and make the backup boundary explicit. Application data is backed up. Containers and generated files are not. I also keep a note describing how to restore each important service. A backup job that reports success is comforting. A restore procedure is evidence.

This setup does not pretend that every disk or node can fail without interruption. Some services will be down while I repair something. I accept that, because extra storage layers would be more systems to understand and maintain.
...
The safe workflow is the same one I use for code. Let the tool propose a change, inspect the diff, validate it, then watch what actually happens. The cluster is small enough that I can keep the whole change in my head.
...
What I ended up with would make a boring cluster diagram. I mean that as a compliment. It gives me hands-on practice and supports daily life without asking to be noticed. I can experiment with a new workload, learn something about Kubernetes, and remove it again without rebuilding the foundation.

I wanted a cluster that still teaches me, and that I can ignore for a week.
